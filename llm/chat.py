...
async def chat_endpoint(req: ChatRequest):
    ...
    if req.action == "chat":
        ...
        plugin_docs = """
            ...
            - 聊天内容涉及数学题、证明题、推理分析等，无需你分析解答，而是表明此时需要调用plugin-reasoner来生成推理过程和答案，而非需要总结文档或者uml图。
        ...
        payload = {
            "model": "deepseek-chat",
            "messages": [
                {"role": "system", "content": "你是一个智能插件调度助手：你将接收到全部聊天历史和插件功能资料。根据所有聊天内容，来判断当前的聊天情境和话题。再据此，来判断当前调用什么插件(可以是0个到多个)，可能可以帮助当前的聊天人。"},
        ...
        try:
            ...
            return JSONResponse({"error": f"deepseek api 调用失败: {str(e)}"}, status_code=500)
        ...
        for line in reply.strip().split('\n'):
            if line.startswith("plugin: plugin-uml"):
                ...
                try:
                    ...
                    print(f"[chat.py] 调用插件 {plugin_path} 失败: {e}", flush=True)
